...
class ReplayBuffer():
    ...
    def sample(self, batch_size):
        ...
        s_shape = self.buffer[0].state.shape # env.observation_space.shape
        
        # Initialize tuple values
        states = torch.zeros(batch_size, s_shape[0], s_shape[1], s_shape[2])
        actions  = torch.zeros(batch_size, 1)
        rewards  = torch.zeros(batch_size, 1)
        next_states = torch.zeros(batch_size, s_shape[0], s_shape[1], s_shape[2])
        dones  = torch.zeros(batch_size, 1)
        
        # Batch index for random sampling
        rand_batch = np.random.choice(len(self.buffer), batch_size, replace=False)
        
        idx = 0
        for batch in rand_batch:
            states[idx] = torch.from_numpy(self.buffer[batch].state)
            actions[idx] = self.buffer[batch].action[0]
            rewards[idx] = self.buffer[batch].reward[0]
            next_states[idx] = torch.from_numpy(self.buffer[batch].next_state)
            dones[idx] = self.buffer[batch].done[0]
            idx +=1
...
def compute_loss(model, target, states, actions, rewards, next_states, dones):
    ...
    '''
    Q_values = model.forward(states).gather(1, actions.long().view(-1,1))
    
    Q_max = target.forward(next_states)
    Q_max = torch.max(Q_max, axis=1)[0].view(len(Q_max), 1)
    Q_max[dones == True] = 0
    
    targets = rewards + gamma * Q_max
    
    loss = nn.MSELoss()
    return loss(Q_values, targets)
...
def train(model_class, env, retrain_mode, max_epsilon=1.0):
    '''
    ...
    for episode in range(max_episodes):
        ...
        for t in range(t_max):
            ...
            col, row = np.where(next_state[1] == 1)

            if done:
                distance_reward = np.array([0])
            else:
                distance_reward = ((w -row) / w + (h - col) / h) / 10.0

            if reward > 0:
                solve_count += 1
                print("get positive reward!", solve_count)

            episode_rewards += reward + distance_reward
            if done:
                break
        ...
        if len(memory) > min_buffer:
            if np.mean(rewards[print_interval:]) < 0.45: #6.0 score settings
            # if np.mean(rewards[print_interval:]) < 0.3:
...
def test(model, env, max_episodes=600):
    ...
    for episode in range(max_episodes):
        ...
        rewards.append(episode_rewards)
        if episode % print_interval == 0 and episode > 0:
            print("{} avg rewards : {:.1f}".format(episode, np.mean(rewards)))
...
def save_bk_model(model):
    data = (model.__class__.__name__, model.state_dict(), model.input_shape, model.num_actions)
    bk_model_path = model_path.replace("model.pt", "model{}.pt".format(int(time.time())))
    torch.save(data, bk_model_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--retrain', dest='retrain', action='store_true', help='retrain the agent')
    ...
    if args.retrain:
        new_reward = 0
        model = get_model()
        original_reward = test(model, env, max_episodes=600)
        print("original_reward", original_reward)
        while new_reward < 9.5:
            print("new loop")
            max_epsilon = 0.2
            if original_reward > 9.0:
                max_epsilon = 0.10
            elif original_reward > 8.5:
                max_epsilon = 0.125
            elif original_reward > 8.0:
                max_epsilon = 0.15
            
            new_model = train(ConvDQN, env, True, max_epsilon)
            new_reward = test(new_model, env, max_episodes=600)
            ...
            if new_reward >= original_reward: # same score also favor trained more model
                print("saving better model")
                save_model(new_model)        
                original_reward = new_reward
            else:
                save_bk_model(new_model)

            env = get_env()

    if args.train:
        model = train(ConvDQN, env, False)
        save_model(model)
    else:
        model = get_model()

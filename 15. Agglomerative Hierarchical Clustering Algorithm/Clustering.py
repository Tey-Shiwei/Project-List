...
def single_linkage(points, clusters, p, q):
    ...
    for i in clusters[p].items: #In ID of P
        for j in clusters[q].items:
            if min_dist > euclidean_distance(points[i], points[j]):
...
def complete_linkage(points, clusters, p, q):
    ...
    for i in clusters[p].items: #In ID of P
        for j in clusters[q].items:
            if max_dist < euclidean_distance(points[i], points[j]):
                max_dist = euclidean_distance(points[i], points[j])
...
class MyAgglomerativeClustering():
            ...
            if self._clusters[i].used:
                for j in range(i + 1, len(self._clusters)):
                    if self._clusters[j].used:
                        if min_dist > self._proximity_matrix[i, j]:
                            min_dist = self._proximity_matrix[i, j]
                            p = i
                            q = j
    ...
    def merge_cluster(self, p, q):
        ...
        '''        
        new_item = MyCluster()
        new_item.extend(self._clusters[p].items+self._clusters[q].items)
        self._clusters.append(new_item)
    ...
    def update_proximity(self, new_cluster):
        ...
        for i in range(len(self._clusters)-1):
            if self._clusters[i].used:
                j = new_cluster-1
                self._proximity_matrix[i, j] = self._linkage_func(self._items, self._clusters, i, j)
        pass

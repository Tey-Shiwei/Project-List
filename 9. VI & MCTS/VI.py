...
def value_iteration(trn_fn, rwd_fn, gamma):
    ...
    delta_U = 1
    Vtemp = np.zeros(trn_fn.shape[0]) # (a)
    value_fn_new = np.zeros_like(value_fn)

    while delta_U > delta:
        delta_U = 0
        value_fn = deepcopy(value_fn_new)        
        for s in range(trn_fn.shape[1]): 
            
            for a in range(trn_fn.shape[0]):
                Vtemp[a] = np.sum(matmul(trn_fn[a,s,:],rwd_fn[s,:] + gamma*value_fn[:]))
                
            bestAction = np.argmax(Vtemp)
            bestValue = np.max(Vtemp)
            value_fn_new[s] = bestValue
            policy[s] = bestAction
            delta_U = max(delta_U,np.abs(value_fn_new[s]-value_fn[s]))
